    ...
    : white_pos(white), black_pos(black)
{
    ...
    if ((white.first > 7) || (white.second > 7)) {
        throw std::domain_error("White queen position out of bounds");
    }
    if ((black.first > 7) || (black.second > 7)) {
        throw std::domain_error("Black queen position out of bounds");
    }
...
bool chess_board::can_attack(void) const
{
    ...
    // Same diagonal
    if ((static_cast<int>(white_row) - static_cast<int>(black_row) < 0 ? (black_row - white_row) : (white_row - black_row)) ==
        (static_cast<int>(white_col) - static_cast<int>(black_col) < 0 ? (black_col - white_col) : (white_col - black_col))) {
        return true;
    }
    return false;
}
