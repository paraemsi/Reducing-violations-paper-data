#include "parallel_letter_frequency.h"
#include <thread>
#include <mutex>
#include <cctype>
#include <algorithm>
#include <map>
#include <vector>
#include <string>

namespace parallel_letter_frequency {

namespace {
    // Helper: count letters in a single string, store in map
    std::map<char, int> count_letters(const std::string& text) {
        std::map<char, int> freq;
        for (char c : text) {
            if (std::isalpha(static_cast<unsigned char>(c))) {
                char lower = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
                ++freq[lower];
            }
        }
        return freq;
    }
}

std::map<char, int> frequency(const std::vector<std::string>& texts) {
    std::vector<std::map<char, int>> partials(texts.size());
    std::vector<std::thread> threads;

    // Mutex not needed for partials, each thread writes to its own index
    for (size_t i = 0; i < texts.size(); ++i) {
        threads.emplace_back([&texts, &partials, i]() {
            partials[i] = count_letters(texts[i]);
        });
    }

    for (auto& t : threads) {
        t.join();
    }

    // Merge all partial maps into a single result
    std::map<char, int> result;
    for (const auto& freq : partials) {
        for (const auto& kv : freq) {
            result[kv.first] += kv.second;
        }
    }
    return result;
}

}
