...
std::vector<unsigned int> convert(unsigned int from_base, const std::vector<unsigned int>& digits, unsigned int to_base) {
    ...
    // Validate digits
    if (!digits.empty()) {
        // Only throw for leading zeros if the number is not all zeros
    ...
    // Special case: empty input means zero (should return empty vector)
    if (digits.empty()) {
        return {};
    }
    // Special case: all digits are zero (should return {0})
    bool all_zeros = std::all_of(digits.begin(), digits.end(), [](unsigned int d){ return d == 0; });
    if (all_zeros) {
        return {0};
    }
