...
namespace meetup {
    ...
    date scheduler::find_teenth_weekday(weekdays wd) const {
        ...
        throw std::domain_error("No such teenth weekday in this month");
    }
    ...
    date scheduler::find_last_weekday(weekdays wd) const {
        ...
        for (int d = last_day; d > last_day - 7; --d) {
            date dt(m_year, m_month, d);
            if (dt.day_of_week() == wd)
                return dt;
        }
        throw std::domain_error("No such last weekday in this month");
    }
