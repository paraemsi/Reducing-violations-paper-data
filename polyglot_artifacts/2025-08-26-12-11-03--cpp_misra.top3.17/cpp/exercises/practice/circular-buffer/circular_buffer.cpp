...
template <typename T>
size_type circular_buffer<T>::capacity() const
{
    return max_size_;
}

template <typename T>
size_type circular_buffer<T>::size() const
{
    return count_;
}
 ...
 * These must be in the header for templates, or in a separate .tpp file included at the end of the header.
 * Remove these from the .cpp file to avoid linker errors.
 */
