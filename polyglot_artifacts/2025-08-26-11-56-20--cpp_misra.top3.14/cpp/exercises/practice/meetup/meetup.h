...
enum class Weekday {
    ...
    Friday,
    Saturday
}; // end enum class Weekday

// MISRA C++:2008 7-1-4: Use enum for Schedule, not enum class, to allow switch/case without qualification.
enum Schedule {
    First,
    Second,
    Third,
...
class scheduler {
public:
    scheduler(month_t month, year_t year) : m_year(year), m_month(month) {}
